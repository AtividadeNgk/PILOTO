...
def save_bot_token_backup(bot_id, original_token):
    """Salva backup do token antes de banir"""
    conn = sqlite3.connect("data.db")
    cursor = conn.cursor()
    
    # Verifica se existe coluna de backup
    cursor.execute("PRAGMA table_info(BOTS)")
    columns = [column[1] for column in cursor.fetchall()]
    
    if 'original_token' not in columns:
        cursor.execute("ALTER TABLE BOTS ADD COLUMN original_token TEXT")
        conn.commit()
    
    cursor.execute("UPDATE BOTS SET original_token = ? WHERE id = ?", (original_token, bot_id))
    conn.commit()
    conn.close()

def get_bot_original_token(bot_id):
    """Recupera o token original do bot"""
    conn = sqlite3.connect("data.db")
    cursor = conn.cursor()
    ...
    cursor.execute("SELECT original_token FROM BOTS WHERE id = ?", (bot_id,))
    result = cursor.fetchone()
    conn.close()

...
async def check_if_banned():
    """Verifica periodicamente se o bot foi banido"""
    while True:
        try:
            bot = manager.get_bot_by_id(bot_application.bot_data['id'])
            if not bot or 'BANIDO' in bot[1]:
                print(f"Bot {bot_application.bot_data['id']} foi banido. Encerrando...")
                # Para todas as tasks
            ...
            # Verifica se o owner foi banido
            owner_id = str(bot[2])
            if manager.bot_banned(owner_id):
                print(f"Owner {owner_id} foi banido. Encerrando bot...")
                await bot_application.stop()
                await bot_application.shutdown()
                import sys
                sys.exit(0)
            ...
            print(f"Erro ao verificar banimento: {e}")
...
async def run_bot(token, bot_id):
    ...
    # VERIFICAÇÃO DE BANIMENTO NO INÍCIO
    # Verifica se o bot foi banido antes de iniciar
    bot = manager.get_bot_by_id(bot_id)
    if not bot or 'BANIDO' in bot[1]:
        print(f"Bot {bot_id} está banido. Não será iniciado.")
        return
    
    # Verifica se o owner está banido
    owner_id = str(bot[2])
    if manager.bot_banned(owner_id):
        print(f"Owner {owner_id} está banido. Bot {bot_id} não será iniciado.")
        return
...
async def main_start(token, id):
    ...
    await asyncio.gather(
        payment_task(),
        expiration_task(),
        check_if_banned()  # ADICIONE ESTA LINHA
    )
